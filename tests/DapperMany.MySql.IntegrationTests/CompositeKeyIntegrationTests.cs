...
/// using MySQL via Testcontainers.
/// </summary>
public class CompositeKeyIntegrationTests : IAsyncLifetime
{
    ...
    {
        var providedConnection = Environment.GetEnvironmentVariable("TEST_MYSQL_CONNECTIONSTRING");
        ...
        else
        {
            _container = new MySqlBuilder()
                .WithImage("mysql:8.0")
                .WithDatabase("dappermany_composite_key_test")
                .WithUsername("root")
        ...
        if (!ready)
            throw new InvalidOperationException($"MySQL container did not become ready within {timeout.TotalMinutes} minutes.");
